...
static void captureCursor(_GLFWwindow *window) {
  XGrabPointer(_glfw.x11.display, window->x11.handle, True,
               ...
               GrabModeAsync, GrabModeAsync, window->x11.handle, None,
               CurrentTime);
}
...
static GLFWbool createNativeWindow(_GLFWwindow *window,
  ...
  window->x11.handle = XCreateWindow(
      _glfw.x11.display, _glfw.x11.root, xpos, ypos, width, height,
      0,     // Border width
      depth, // Color depth
...
static Atom writeTargetToProperty(const XSelectionRequestEvent *request) {
  ...
  if (request->target == _glfw.x11.MULTIPLE) {
    ...
    const unsigned long count = _glfwGetWindowPropertyX11(
        request->requestor, request->property, _glfw.x11.ATOM_PAIR,
    ...
    if (window->x11.ic) {
      ...
      if (!filtered) {
        ...
        if (status == XBufferOverflow) {
          ...
          count = Xutf8LookupString(window->x11.ic, &event->xkey, chars, count,
                                    NULL, &status);
        }
...
void _glfwCreateInputContextX11(_GLFWwindow *window) {
  ...
  window->x11.ic = XCreateIC(
      _glfw.x11.im, XNInputStyle, XIMPreeditNothing | XIMStatusNothing,

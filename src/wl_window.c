...
static void createFallbackDecorations(_GLFWwindow *window) {
  ...
  createFallbackDecoration(window, &window->wl.decorations.top,
                           window->wl.surface, window->wl.decorations.buffer, 0,
                           -GLFW_CAPTION_HEIGHT, window->wl.width,
                           GLFW_CAPTION_HEIGHT);
  createFallbackDecoration(
      window, &window->wl.decorations.left, window->wl.surface,
      window->wl.decorations.buffer, -GLFW_BORDER_SIZE, -GLFW_CAPTION_HEIGHT,
      GLFW_BORDER_SIZE, window->wl.height + GLFW_CAPTION_HEIGHT);
  createFallbackDecoration(
      window, &window->wl.decorations.right, window->wl.surface,
      window->wl.decorations.buffer, window->wl.width, -GLFW_CAPTION_HEIGHT,
      GLFW_BORDER_SIZE, window->wl.height + GLFW_CAPTION_HEIGHT);
  createFallbackDecoration(
      window, &window->wl.decorations.bottom, window->wl.surface,
...
static void
xdgDecorationHandleConfigure(void *userData,
...
static void surfaceHandleEnter(void *userData, struct wl_surface *surface,
  ...
  if (window->wl.scaleCount + 1 > window->wl.scaleSize) {
    window->wl.scaleSize++;
    window->wl.scales = _glfw_realloc(
        window->wl.scales, window->wl.scaleSize * sizeof(_GLFWscaleWayland));
  }

  window->wl.scaleCount++;
  window->wl.scales[window->wl.scaleCount - 1].factor =
      monitor->wl.contentScale;
...
void libdecorFrameHandleConfigure(struct libdecor_frame *frame,
  ...
  if (!libdecor_configuration_get_content_size(config, frame, &width,
                                               &height)) {
...
void libdecorFrameHandleDismissPopup(struct libdecor_frame *frame,
                                     const char *seatName, void *userData) {}

static const struct libdecor_frame_interface libdecorFrameInterface = {
    libdecorFrameHandleConfigure, libdecorFrameHandleClose,
    libdecorFrameHandleCommit, libdecorFrameHandleDismissPopup};

static GLFWbool createLibdecorFrame(_GLFWwindow *window) {
  window->wl.libdecor.frame =
      libdecor_decorate(_glfw.wl.libdecor.context, window->wl.surface,
...
static void handleEvents(double *timeout) {
  ...
  struct pollfd fds[4] = {{wl_display_get_fd(_glfw.wl.display), POLLIN},
    ...
    if (fds[1].revents & POLLIN) {
      ...
      if (read(_glfw.wl.keyRepeatTimerfd, &repeats, sizeof(repeats)) == 8) {
        for (uint64_t i = 0; i < repeats; i++) {
          _glfwInputKey(
              _glfw.wl.keyboardFocus, translateKey(_glfw.wl.keyRepeatScancode),
...
static void dataDeviceHandleDataOffer(void *userData,
  ...
  _GLFWofferWayland *offers = _glfw_realloc(
      _glfw.wl.offers, sizeof(_GLFWofferWayland) * (_glfw.wl.offerCount + 1));
  if (!offers) {
    _glfwInputError(GLFW_OUT_OF_MEMORY, NULL);
    return;
  }
...
void _glfwSetWindowIconWayland(_GLFWwindow *window, int count,
...
void _glfwSetWindowSizeLimitsWayland(_GLFWwindow *window, int minwidth,
  ...
  if (window->wl.libdecor.frame) {
    ...
    libdecor_frame_set_min_content_size(window->wl.libdecor.frame, minwidth,
                                        minheight);
    libdecor_frame_set_max_content_size(window->wl.libdecor.frame, maxwidth,
                                        maxheight);
  } else if (window->wl.xdg.toplevel) {
    if (minwidth == GLFW_DONT_CARE || minheight == GLFW_DONT_CARE)
      minwidth = minheight = 0;
...
void _glfwSetWindowAspectRatioWayland(_GLFWwindow *window, int numer,
                                      int denom) {
  if (window->wl.maximized || window->wl.fullscreen)
    return;
...
void _glfwGetFramebufferSizeWayland(_GLFWwindow *window, int *width,
                                    int *height) {
...
void _glfwGetWindowFrameSizeWayland(_GLFWwindow *window, int *left, int *top,
...
void _glfwGetWindowContentScaleWayland(_GLFWwindow *window, float *xscale,
                                       float *yscale) {
  if (xscale)
...
void _glfwShowWindowWayland(_GLFWwindow *window) {
  if (!window->wl.libdecor.frame && !window->wl.xdg.toplevel) {
    // NOTE: The XDG surface and role are created here so command-line
    // applications
...
void _glfwSetWindowMonitorWayland(_GLFWwindow *window, _GLFWmonitor *monitor,
...
void _glfwSetWindowMousePassthroughWayland(_GLFWwindow *window,
                                           GLFWbool enabled) {
  if (enabled) {
    struct wl_region *region = wl_compositor_create_region(_glfw.wl.compositor);
...
GLFWbool _glfwCreateCursorWayland(_GLFWcursor *cursor, const GLFWimage *image,
                                  int xhot, int yhot) {
...
GLFWbool _glfwCreateStandardCursorWayland(_GLFWcursor *cursor, int shape) {
  ...
  if (!cursor->wl.cursor) {
    ...
    if (!cursor->wl.cursor) {
      _glfwInputError(GLFW_CURSOR_UNAVAILABLE,
                      "Wayland: Failed to create standard cursor \"%s\"", name);
      return GLFW_FALSE;
    }
...
static void
lockedPointerHandleLocked(void *userData,
                          struct zwp_locked_pointer_v1 *lockedPointer) {}

static void
lockedPointerHandleUnlocked(void *userData,
                            struct zwp_locked_pointer_v1 *lockedPointer) {}
...
static void confinedPointerHandleConfined(
    void *userData, struct zwp_confined_pointer_v1 *confinedPointer) {}

static void confinedPointerHandleUnconfined(
    void *userData, struct zwp_confined_pointer_v1 *confinedPointer) {}
...
static void confinePointer(_GLFWwindow *window) {
  ...
  zwp_confined_pointer_v1_add_listener(window->wl.confinedPointer,
                                       &confinedPointerListener, window);
}
...
GLFWbool _glfwGetPhysicalDevicePresentationSupportWayland(
